...
module.exports = {
		...
		var sql = "SELECT * FROM event;SELECT * FROM notice as n,student as s,subject as sb WHERE s.class_id=n.class_id AND s.section_id=n.section_id AND sb.subject_id=n.subject_id AND s.student_id=? ORDER BY n.noticedate DESC;select * from student where student_id=?";
	...
	graderesult: function(user, callback){
		var sql = "SELECT * FROM result as R,grade as G,subject as S,student as ST WHERE ST.class_id=R.class_id AND ST.section_id=R.section_id AND S.subject_id=R.subject_id  AND ST.student_id=?";
		db.getResults(sql,user, function(result){
